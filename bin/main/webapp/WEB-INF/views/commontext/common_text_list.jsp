...
function doRemove(){
	...
	if(confirm("您确定要删除所选的记录吗？")){
		 $.ajax({
		  	   type: 'post', 
		  	   url:'<c:url value='/commonText/removeCommonText'/>',
		  	   data:$('#commonText').serialize(),
...
function doRemoveOne(ctid){
	...
	if(confirm("您确定要删除所选的记录吗？")){
		 $.ajax({
		  	   type: 'post', 
		  	   url:'<c:url value='/commonText/removeCommonText'/>?ctid='+ctid,
		  	   success:function(data){
		  		   if(data.success){
		  			   window.location = '<c:url value='/commonText/list?currpage=1'/>';
		  		   }else{
        ...
        <TBODY>
          <TR>
            <TD vAlign=top width="100%" bgColor=#ffffff>
              ...
              <TR>
                <TD class=manageHead>当前位置：管理首页 &gt; 公共文本管理模块 &gt; 公共文本列表</TD>
              </TR>
              <TR>
                <TD height=2></TD>
              </TR>
                ...
                <TBODY>
                 <!-- 数据筛选区域，查询表单区域 -->
                  <TR>
	                  <TD height="35px">
		                  <!-- 工具栏区域，主要是快捷按钮，如添加，删除，导入，导出 -->
		                 	 ...
		                 	 <INPUT class=button id="back" type="button" value=" 添  加 " onclick="javascript:window.location='<c:url value='/commonText/createCommonText' />'" />
                  ...
                  <TR>
                    <TD height="35px">
                    <!-- 数据筛选区域，查询表单区域 -->
                    <FORM id="do_search_form" name="do_search_form" method=post action="<c:url value="/commonText/list"/>" >
                    ...
                    <TBODY>
                     	<TR>
                            ...
                            <TD>
                            	 代码
                             	 <INPUT type="text" id="code" name="code" style="WIDTH: 100px" maxLength=50  value="${commonText.code}"/>  &nbsp;&nbsp;
                             	  名称
                             	 <INPUT type="text" id="value" name="value" style="WIDTH: 100px" maxLength=50  value="${commonText.value}"/>  &nbsp;&nbsp;
                            </TD>
                  ...
                  <TR>
                    ...
                    <!-- 数据列表，使用el标签循环生成即可 -->
                    <FORM id="commonText" name="commonText" method=post>
                    <TABLE id=grid style="BORDER-TOP-WIDTH: 0px; FONT-WEIGHT: normal; BORDER-LEFT-WIDTH: 0px; BORDER-LEFT-COLOR: #cccccc; BORDER-BOTTOM-WIDTH: 0px; BORDER-BOTTOM-COLOR: #cccccc; WIDTH: 100%; BORDER-TOP-COLOR: #cccccc; FONT-STYLE: normal; BACKGROUND-COLOR: #cccccc; BORDER-RIGHT-WIDTH: 0px; TEXT-DECORATION: none; BORDER-RIGHT-COLOR: #cccccc" cellSpacing=1 cellPadding=2 rules=all border=0>
                        ...
                        <TR style="FONT-WEIGHT: bold; FONT-STYLE: normal; BACKGROUND-COLOR: #eeeeee; TEXT-DECORATION: none" height="30px">
                   		...
                   		<TR style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none" height="30px">
                            <TD class="list_td_align"><INPUT name="chk" type="checkbox" value="${list.ctid }" /></TD>
                            ...
                            <TD class="list_td_align">
                            	<A href="javascript:void(0)" onclick="doRemoveOne(${list.ctid })">删除</A> 
                            </TD>
                         </TR>
                   		</c:forEach>
