...
function jq(){
	...
	if(typeof arguments[0] !="string"){
		...
		elem = arguments[0];
		if(!elem["version"]){
			jq._Method.Element.apply(elem);
			if(jq._appendMethod){
				jq._appendMethod.apply(elem);
			}
		}
		return elem;
	}
	...
	var path = argID.replace(/(^,*)|(,*jq)/g,"").split(",");
	var allelem = [];
	for(var a=0,b; b=path[a]; a++){
		...
		for (var i=0,q; q=p[i]; i++){
			if (q.indexOf("#")==0){
				...
				elem = jq(q.substring(1)); 
				continue;
			}
			var attsel = [];
			var elem_temp=[];
			if (q.indexOf(".")!=-1){
				var tags = q.replace(/\[.*?\]/gi,function(jq1){attsel.push(jq1.replace(/\[|\]/g,""));return "";});
				var tag = tags.split(".")[0];
				var cn = tags.split(".")[1];
				if (elem == null){
					elem_temp = jq._find(tag,cn,arguments[1] || document);
				}else{
					if (elem instanceof Array){
						var arr = [];
						elem.each(function(obj){jq._find(tag,cn,obj).each(function(){arr.push(arguments[0])});});						
						elem_temp = arr;
					}else{
						elem_temp = jq._find(tag,cn,elem);
					}
				}
				elem = jq._attributeSelector(attsel,elem_temp);
				continue;
			}else{
				var tag = q.replace(/\[.*?\]/gi,function(jq1){attsel.push(jq1.replace(/\[|\]/g,""));return "";});
				if(elem == null){
					elem_temp = jqA((arguments[1] || document).getElementsByTagName(tag)).each(function(obj){jq(obj)});
				}else{
					if (elem instanceof Array){
						var arr = [];
						elem.each(function(obj){jqA(obj.getElementsByTagName(tag)).each(function(obj){arr.push(jq(obj))})});
						elem_temp = arr;
					}else{
						elem_temp = jqA(elem.getElementsByTagName(tag)).each(function(obj){jq(obj)});
					}
				}
				elem = jq._attributeSelector(attsel,elem_temp);
			}
...
jq.Version = "0.2.1.5";
jq._find = function(tag,cn,par){
	var arr = par.getElementsByTagName(tag||"*");
	var elem = [];
	for(var i=0,j; j=arr[i]; i++){
		if(j.className.hasSubString(cn," ")){elem.push(jq(j));}
	}
	return elem;
};
jq._attributeSelector = function(attsel,elem_temp){
...
jq._Method = {
	Element	: function(){
		this.version = jq.Version;
		this.hide = function(){this.style.display="none"; return this};
		this.show = function(){this.style.display=""; return this};
		this.getStyle = function(s){
			var value = this.style[s=="float"?(jq.Browse.isIE()?"styleFloat":"cssFloat"):s.camelize()];
			if (!value){
		...
		this.setStyle = function(s){
			...
			for (var i=0,j; j=sList[i]; i++){
				var k = j.split(/:(?!\/\/)/g);
				var key = k[0].trim();
				key=key=="float"?(jq.Browse.isIE()?"styleFloat":"cssFloat"):key.camelize();
		...
		this.subTag = function(){return jqA(this.getElementsByTagName(arguments[0])).each(function(n){jq(n);});};
	...
	String : function(){
		this.trim = function(){
			...
			if(typeof(_argument)=="string"){
				return this.replace(_argument == " "?/(^\s*)|(\s*jq)/g : new RegExp("(^"+_argument+"*)|("+_argument+"*jq)","g"),"");
		...
		this.camelize = function(){
			return this.replace(/(-\S)/g,function(jq1){return jq1.toUpperCase().substring(1,2)});
		};
	...
	Function : function(){
		this.bind = function() {
  			var __method = this, args = jqA(arguments), object = args.shift();
  			return function() {
    			return __method.apply(object, args.concat(jqA(arguments)));
  			}
		};
	},
...
jq._Method.Array.apply(Array.prototype);
jq._Method.String.apply(String.prototype);
jq._Method.Function.apply(Function.prototype);
jq._Method.StringBuffer.apply(StringBuffer.prototype);
jq._Method.NameSpace.apply(NameSpace.prototype);
...
jq.Browse = {
	...
	IEVer : function(){return jq.Browse.isIE() ? parseInt(navigator.userAgent.split(";")[1].trim().split(" ")[1]) : 0;}
};
jq(document);
...
jq.Ajax = {
	...
	get	: function(url,complete,error){ return jq.Ajax.Request(url+(url.indexOf("?")==-1?"?":"&")+Math.random(),"get","",complete,error); },
	post : function(url,para,complete,error){ return jq.Ajax.Request(url,"post",para,complete,error);},
	...
	update : function(url,id){ return jq.Ajax.Request(url,(arguments[2]?"post":"get"),(arguments[2]?arguments[2]:Math.random()),function(x){if("INPUT,SELECT,BUTTON,TEXTAREA".hasSubString(jq(id).tagName,",")){jq(id).value=x.responseText;}else{jq(id).innerHTML=x.responseText;}});}
};
jq.Cookies = {
    get : function(n){
...
jq.Form = function(n){
	...
	jq.Form._Method.apply(f);
	return f;
};
jq.Form._Method = function(){
	this.serialize = function(obj){
			...
			for(var i=0,j; j=arr[i]; i++){
				...
				elem[na].push(jqE(j.value));
			}
			return jq.Form.serialize(obj,elem);
		};
};
jq.Form.serialize = function(obj){
...
jq.Redirect = function(url,paraStr){
	if(!paraStr){
...
jq.DOMReady = function(f){
	jq.DOMReady._methodArgument = f;
	return jq.DOMReady.checkReady();
};
jq.DOMReady.checkReady = function(){
	if(document&&document.getElementsByTagName&&document.getElementById&&document.body){
		return jq.DOMReady._methodArgument();
	}
	setTimeout("jq.DOMReady.checkReady()",10);
	return null;
}
...
function jqD(str){return unescape(str);};//decodeURIComponent(str)
function jqE(str){return escape(str);};//encodeURIComponent(str)
function jqV(id){return jq(id)?(jq(id).tagName.hasSubStrInArr(["INPUT","TEXTAREA","SELECT","BUTTON"])?jq(id).value : jq(id).innerHTML):"";};
window.recycler=(function(){ var t=document.createElement('div');t.id="recycler";return t;})();

//--byMoo------------------------------------------------
//取得对象绝对位置
//返回,e._top=绝对顶边距;e._left=绝对左边距;
function jqTL(e){
	var o=e=jq(e);if(!e) return;
	var top=o.offsetTop;var left=o.offsetLeft;
	while(e=e.offsetParent){
		top+=e.offsetTop;left+=e.offsetLeft;
	}e=null;
	o._top=top;o._left=left;o=null;
};