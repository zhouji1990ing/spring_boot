...
$(function(){
	...
	function Do_save_ftn(){
		...
		data.append('code','${constants.code}');
		data.append('value',$('#value').val());
        
		$.ajax({
			url:'<c:url value='/constants/modifyConstants'/>',
		  	type: 'post', 
		  	data:data,
       	    cache: false,
        ...
        <TBODY>
          <TR>
            <TD vAlign=top width="100%" bgColor=#ffffff>
              ...
              <TR>
                <TD class=manageHead>当前位置：管理首页 &gt; 常量管理模块 &gt; 编辑常量信息</TD>
              </TR>
              <TR>
                <TD height=2></TD>
              </TR>
                    ...
                    <TABLE id=grid class="page_add" border=0>
                      <TBODY>
                      	<tr>
						     <td width="10%">代码：</td>
						     <td width="86%">
						     <input type="hidden" name="cid" id="cid"  class="input_wb" value="${constants.cid }" />
						     	${constants.code }
						     </td>
						 </tr>
						 <tr>
						     <td width="10%">名称：</td>
						     <td width="86%">
						     	<input type="text" name="value" id="value"  class="input_wb" value="${constants.value }" />
